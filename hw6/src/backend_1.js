...
export const fetchArticles = () => {
    return (dispatch) => {
        resource('GET', 'articles')
            .then( r => {
                dispatch({type: Actions.UPDATE_CONTENT, contents: r.articles })
            })
            .catch(r => {
                Actions.setErrorMsg("Cannot get any articles")(dispatch)
...
export const updateHeadline = (headline) => (dispatch) => {
    resource("PUT", "headline", {
        ...
        .then(r => {
            dispatch({type:Actions.UPDATE_HEADLINE, text: headline})
        })
        .catch(r => {
...
export const loginFetch = (username, password) => (dispatch) => {
    resource('POST', 'login', {
        ...
        .then(r => {
            dispatch({type: Actions.LOGIN, headline: r.headlines[0]})
        })
        .then(r => resource('GET', 'avatars'))
        .then(r => dispatch({type: Actions.UPDATE_AVATAR ,field: r.avatars[0].avatar}))
        .then(r => {
